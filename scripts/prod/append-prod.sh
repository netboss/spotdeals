...
cleanup() {
  ...
  "$DRUSH" migrate:reset-status spotdeals_venues >/dev/null 2>&1
  "$DRUSH" migrate:reset-status spotdeals_deals >/dev/null 2>&1
  "$DRUSH" php:script scripts/spotdeals_append_new_csv_rows.php -- --dataset=food --action=restore
...
"$DRUSH" php:script scripts/spotdeals_append_new_csv_rows.php -- --dataset=food --action=prepare
